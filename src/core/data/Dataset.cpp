...
std::pair<nn::Tensor, nn::Tensor> Dataset::get_batch(const std::vector<int>& indices) const {
    if (indices.empty()) {
    ...
    nn::Tensor batch_features(batch_size, feature_size);
    nn::Tensor batch_labels(batch_size, label_size);
...
std::pair<nn::Tensor, nn::Tensor> Dataset::get_batch(int start, int batch_size) const {
...
nn::Tensor Dataset::compute_std() const {
    if (!loaded_ || samples_.empty()) {
        throw std::runtime_error("Dataset not loaded or empty");
    }
    
    nn::Tensor mean = compute_mean();
    int feature_size = samples_[0].features.cols();
    nn::Tensor variance(1, feature_size);
    variance.zero();
    
    // Compute variance
    for (const auto& sample : samples_) {
        nn::Tensor diff = sample.features - mean;
        for (int j = 0; j < feature_size; ++j) {
    ...
    nn::Tensor std_dev(1, feature_size);
    for (int j = 0; j < feature_size; ++j) {

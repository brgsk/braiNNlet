...
Network::Network() 
    : loss_function_(create_loss(LossType::MeanSquaredError))
{
}
...
void Network::remove_layer(int index) {
    ...
    if (index == -1) {
        ...
        // Remove specific layer by index
...
void Network::validate_network() const {
    ...
    for (size_t i = 1; i < layers_.size(); ++i) {
        ...
        if (prev_output != curr_input) {
            throw std::runtime_error("Layer " + std::to_string(i-1) + 
                                   " output size (" + std::to_string(prev_output) + 
                                   ") doesn't match layer " + std::to_string(i) + 
                                   " input size (" + std::to_string(curr_input) + ")");
        }
    }

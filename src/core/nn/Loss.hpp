...
public:
    ...
    virtual double forward(const Tensor& predictions, const Tensor& targets) const = 0;
    virtual Tensor backward(const Tensor& predictions, const Tensor& targets) const = 0;
...
public:
    double forward(const Tensor& predictions, const Tensor& targets) const override;
    Tensor backward(const Tensor& predictions, const Tensor& targets) const override;
    std::string name() const override { return "MSE"; }
    LossType type() const override { return LossType::MeanSquaredError; }
};
...
public:
    ...
    LossType type() const override { return LossType::CrossEntropy; }
...
public:
    ...
    LossType type() const override { return LossType::BinaryCrossEntropy; }
};

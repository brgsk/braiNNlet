...
struct TrainingConfig {
    ...
    TrainingConfig(int e, int bs, double lr) 
        : epochs(e), batch_size(bs), learning_rate(lr) {}
};

// Callback function types
using OnEpochEndCallback = std::function<void(int epoch, const TrainingMetrics& train_metrics, const TrainingMetrics& val_metrics)>;
...
public:
    ...
    // Callbacks
    void set_on_epoch_end(OnEpochEndCallback callback) { on_epoch_end_ = callback; }
    void set_on_batch_end(OnBatchEndCallback callback) { on_batch_end_ = callback; }
    void set_on_training_start(OnTrainingStartCallback callback) { on_training_start_ = callback; }
    void set_on_training_end(OnTrainingEndCallback callback) { on_training_end_ = callback; }
    ...
    // History
    const std::vector<TrainingMetrics>& training_history() const { return training_history_; }
    const std::vector<TrainingMetrics>& validation_history() const { return validation_history_; }
...
private:
    ...
    // Helper methods
    void train_epoch(const std::vector<int>& train_indices, const TrainingConfig& config, int epoch);

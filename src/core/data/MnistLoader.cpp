...
namespace data
{
    ...
    bool MnistDataset::load_train_test_split(const std::string &mnist_dir, double train_ratio)
    {
        try
        {
            ...
            if (train_ratio < 1.0)
            {
                ...
                std::cout << "Using " << keep_size << " samples (ratio: " << train_ratio << ")" << std::endl;
            }
    ...
    void MnistDataset::create_validation_split(double validation_ratio)
    {
        ...
        std::cout << "Dataset shuffled for validation split (ratio: " << validation_ratio << ")" << std::endl;
    }
    ...
    std::vector<std::vector<uint8_t>> MnistDataset::load_images(const std::string &filename)
    {
        ...
        std::cout << "MNIST Images: " << num_images << " samples, "
                  << num_rows << "x" << num_cols << " pixels" << std::endl;

...
void Trainer::train(const TrainingConfig& config) {
    ...
    int batches_per_epoch = static_cast<int>((train_indices.size() + config.batch_size - 1) / config.batch_size);
...
void Trainer::train_epoch(const std::vector<int>& train_indices, const TrainingConfig& config, int epoch) {
    network_.set_training(true);
    
    int num_batches = static_cast<int>((train_indices.size() + config.batch_size - 1) / config.batch_size);
    
    for (int batch_idx = 0; batch_idx < num_batches && !should_stop_; ++batch_idx) {
        // Get batch indices
        int start_idx = batch_idx * config.batch_size;
        int end_idx = std::min(start_idx + config.batch_size, static_cast<int>(train_indices.size()));
